...
function glu_emp_model(glc_ext_input)
    ...
    glu_emp = Model(
        optimizer_with_attributes(KNITRO.Optimizer, "ms_enable" => 1, "ms_maxsolves" => 50),
    )
    ...
    @NLconstraints glu_emp begin
        ...
        dg_akgsyn ==
        dG0_akgsyn + RT * (akg + 2 * co2 + nadh + nadph - nad - nadp - pyr - oac)
        ...
        glnsyn +
        co2_diff +
        nh3_diff <= total_proteome_mass_fraction
